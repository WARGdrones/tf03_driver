...
TF03::TF03(const std::string & node_name)
: rclcpp::Node(node_name)
{
  timeout_ms = 1000;
  this->declare_parameter<std::string>("serial_port", "/dev/ttyUSB0");
  this->declare_parameter<std::string>("can_device", "panther_can");
  this->declare_parameter<int>("can_receive_id", 0x3003);
  this->declare_parameter<std::string>("sensor_interface", "serial");
  this->declare_parameter<bool>("print_version", false);
  this->declare_parameter<std::string>("set_output_format", "");
  this->declare_parameter<int>("set_transmit_can_id", 0);
  this->declare_parameter<int>("set_receive_can_id", 0);

  this->get_parameter("serial_port", serial_port);
  this->get_parameter("can_device", can_device);
  this->get_parameter("can_receive_id", can_receive_id);
  this->get_parameter("sensor_interface", sensor_interface);
  this->get_parameter("print_version", print_version);
  this->get_parameter("set_output_format", set_output_format);
  this->get_parameter("set_transmit_can_id", set_transmit_can_id);
  this->get_parameter("set_receive_can_id", set_receive_can_id);
  ...
  if (set_transmit_can_id != 0) {
    ...
    parameters.push_back(
      parameter_config{false, false, tf_03_command_id::transmit_can_id, set_transmit_can_id});
  }
  if (set_receive_can_id != 0) {
    reconfigure_sensor = true;
    RCLCPP_INFO(this->get_logger(), "New value for set_receive_can_id %#4x", set_receive_can_id);
    parameters.push_back(
      parameter_config{false, false, tf_03_command_id::receive_can_id, set_receive_can_id});
  }
  if (!set_output_format.empty()) {
    if (set_output_format == "serial") {
      RCLCPP_INFO(
        this->get_logger(), "New value for set_output_format %s", set_output_format.c_str());
      reconfigure_sensor = true;
      parameters.push_back(
        parameter_config{false, false, tf_03_command_id::output_format, SET_OUTPUT_FORMAT_SERIAL});
    } else if (set_output_format == "can") {
      RCLCPP_INFO(
        this->get_logger(), "New value for set_output_format %s", set_output_format.c_str());
      reconfigure_sensor = true;
      parameters.push_back(
        parameter_config{false, false, tf_03_command_id::output_format, SET_OUTPUT_FORMAT_CAN});
    } else {
      RCLCPP_ERROR(
        this->get_logger(),
        "Invalid value for parameter set_output_format. Valid are serial, can.");
    }
  }
  if (sensor_interface == "serial") {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Will use CAN interface '%s' with device ID %#04x", can_device.c_str(),
      can_receive_id);
  } else {
  ...
  if (
    init_sensor(interface, (interface == tf_03_interface::serial) ? serial_port : can_device) !=
    0)
  {
    RCLCPP_ERROR(this->get_logger(), "Cannot init sensor!");
    return;
  }
  ...
  if (reconfigure_sensor) {
    ...
    while (!parameters.empty()) {
      if (!parameters[0].frame_sent) {
        send_command(interface, parameters[0].command, parameters[0].argument);
        command_timestamp = this->now();
        parameters[0].frame_sent = true;
      } else if ((this->now() - command_timestamp).seconds() > 2.0) {
    ...
    for (auto s : sensors) {
      sensor_pub.insert(
        std::pair<int, rclcpp::Publisher<sensor_msgs::msg::Range>::SharedPtr>(
          s.first, this->create_publisher<sensor_msgs::msg::Range>("sensor/" + s.second, 1)));
      sensor_data.insert(
        std::pair<int, sensor_msgs::msg::Range>(s.first, sensor_msgs::msg::Range()));
...
void TF03::process_sensor_data()
{
  switch (interface) {
    case tf_03_interface::serial:
      // RCLCPP_INFO(this->get_logger(), "Case serial");
      try {
      ...
      if (is_buffer_correct(&incoming_buffer)) {
        ...
        if (incoming_buffer.size() > 9) {
          RCLCPP_ERROR(
            this->get_logger(), "Buffer not correct and oversized, size=%ld",
...
void TF03::clear_incoming_buffer() {incoming_buffer.clear();}
...
void TF03::process_incoming_buffer(std::vector<u_char> data, int can_id)
{
  if (data.size() == 6 && data[4] == 0 && data[5] == 0 && !reconfigure_sensor) {
    ...
    if (data[2] == tf_03_command_id::version) { // Command: Version nnumber
      ...
      RCLCPP_INFO(
        this->get_logger(), "Version is %d.%d.%d", (uint16_t)data[5], (uint16_t)data[4],
        (uint16_t)data[3]);
      if (parameters[0].command == tf_03_command_id::version) {
        parameters[0].command_success = true;
      } else {
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::version, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::system_reset) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::system_reset, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::output_format) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::output_format, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::transmit_can_id) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::transmit_can_id, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::receive_can_id) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::receive_can_id, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::save_settings) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::save_settings, parameters[0].command);
      }
      ...
      if (parameters[0].command == tf_03_command_id::factory_settings) {
        ...
        RCLCPP_WARN(
          this->get_logger(), "Expected confirmation for %#2x, received %#2x instead",
          tf_03_command_id::factory_settings, parameters[0].command);
      }
    }
  } else {
    // RCLCPP_INFO(this->get_logger(), "Data not correct");
  }
}
...
int TF03::init_sensor(tf_03_interface i, std::string device_name)
{
  if (i == tf_03_interface::serial) {
    // RCLCPP_INFO(this->get_logger(), "Create serial port object");
#ifdef LIBSERIAL_0_X
    // RCLCPP_INFO(this->get_logger(), "Using LIBSERIAL_0_x");
    tf03_serial_port = new SerialPort(device_name);
    if (!tf03_serial_port->IsOpen()) {
      // RCLCPP_INFO(this->get_logger(), "Open serial port");
      try {
        tf03_serial_port->Open(
          SerialPort::BaudRate::BAUD_115200, SerialPort::CharacterSize::CHAR_SIZE_8,
          SerialPort::Parity::PARITY_NONE, SerialPort::StopBits::STOP_BITS_1,
...
#else
    // RCLCPP_INFO(this->get_logger(), "Using LIBSERIAL_1_x");
    tf03_serial_port = new LibSerial::SerialPort(
      device_name, LibSerial::BaudRate::BAUD_115200, LibSerial::CharacterSize::CHAR_SIZE_8,
      LibSerial::FlowControl::FLOW_CONTROL_NONE, LibSerial::Parity::PARITY_NONE,
    ...
    if (can_socket == -1) {
      ...
      // RCLCPP_INFO(this->get_logger(), "CAN socket ID: %ld", can_socket);
    }
    ...
    // RCLCPP_INFO(this->get_logger(), "Init frame");
...
void TF03::write_command_data(tf_03_interface i, std::vector<u_char> data)
{
  switch (i) {
    ...
    case tf_03_interface::can:
      ...
      // RCLCPP_INFO(this->get_logger(), "Sent %d bytes", nbytes);
      break;
  }
}
...
{
  // RCLCPP_INFO(this->get_logger(), "verify_checksum() %d", data[data.size()]);
  // if (data.size() == 7)
  // {
  // RCLCPP_INFO(this->get_logger(), "%d, %d, %d, %d, %d, %d, %d", data[0], data[1], data[2], data[3], data[4], data[5], data[6]);
  // }
  if (
    get_checksum(std::vector<u_char>(&data[0], &data[data.size() - 1])) == data[data.size() - 1])
  {
    // RCLCPP_INFO(this->get_logger(), "verify_checksum: True");
    return true;
  }
  // RCLCPP_INFO(this->get_logger(), "verify_checksum: False");
  return false;
}

u_char TF03::get_checksum(std::vector<u_char> data)
{
  // RCLCPP_INFO(this->get_logger(), "get_checksum()");
  u_char checksum = 0;
  for (auto c : data) {
    // RCLCPP_INFO(this->get_logger(), "Byte %d", (int)c);
    checksum += c;
  }
  // RCLCPP_INFO(this->get_logger(), "return checksum: %d", checksum);
  return checksum;
}

std::map<tf_03_command_id, u_char> TF03::tf_03_command_len = {
  {version, 4}, {system_reset, 4}, {framerate, 6}, {factory_settings, 4},
  {save_settings, 4}, {output_format, 5}, {transmit_can_id, 0x08}, {receive_can_id, 0x08}};
